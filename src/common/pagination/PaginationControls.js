    ...
    return <div className="m-2">
      ...
      <div className="flex justify-center border-b border-gray-300">
        ...
        <div className="flex-none text-center py-2 pt-0 m-2">
          <div>
            <select className="text-blue font-semibold bg-gray-200 mt-1 form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5"
              onChange={this.handlePageSizeChange}
              value={this.props.pageSize || this.pageSizes[0]}>
              {this.pageSizes.map(s =>
                <option
                  className="text-blue" 
                  value={s} key={s}>{s} per page</option>
              )}
            </select>
          </div>
        </div>
